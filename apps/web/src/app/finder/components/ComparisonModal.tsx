          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-800 text-sm">
                ...
                {products.map(p => (
                  <td key={p.id} className="p-4">
                    <div className="space-y-1">
                      ...
                      <div className="text-xs text-center text-slate-500 mt-1">ROI: {p.roi ?? 0}x</div>
                    </div>
                  </td>
                ))}
              </tr>
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          ...
                          <span className={(p.saturation ?? 0) > 60 ? 'text-rose-400 font-bold' : 'text-emerald-400 font-bold'}>{p.saturation ?? 0}%</span>
                        </div>
                        <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${(p.saturation ?? 0) > 60 ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{ width: `${p.saturation ?? 0}%` }}></div>
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-slate-500">Competition</span>
                          <span className="text-blue-400 font-bold">{p.competition ?? 0}%</span>
                        </div>
                        <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                          <div className="h-full rounded-full bg-blue-500" style={{ width: `${p.competition ?? 0}%` }}></div>
                        </div>
                      </div>
                      <div className="flex items-center gap-1 text-xs text-slate-400">
                        <ShoppingBag size={12} /> {p.shopifyStoreCount ?? 0} Active Stores
                      </div>
                    </div>
                  </td>
                ))}
                ...
                {products.map(p => (
                  <td key={p.id} className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`text-2xl font-bold ${(p.viralScore ?? 0) > 80 ? 'text-purple-400' : 'text-slate-400'}`}>{p.viralScore ?? 0}</div>
              ...
              <tr className="hover:bg-slate-800/30 transition-colors">
                ...
                {products.map(p => (
                  <td key={p.id} className="p-4">
                    <p className="text-xs text-slate-400 italic leading-relaxed">"{p.winningReason ?? 'N/A'}"</p>
                  </td>
                ))}
              </tr>
              ...
              <tr className="hover:bg-slate-800/30 transition-colors">
                ...
                {products.map(p => {
                  const bestSupplier = p.suppliers && p.suppliers.length > 0 ? p.suppliers[0] : null;
                  return (
                    <td key={p.id} className="p-4">
                      {bestSupplier ? (
