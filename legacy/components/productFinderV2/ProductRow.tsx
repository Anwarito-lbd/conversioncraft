...
const ProductRow: React.FC<ProductRowProps> = ({ product, onClick, selected, onToggleSelect }) => {
  ...
  return (
    ...
    >
        ...
        <td className="py-4 px-6 text-slate-300">{product.roi}x</td>
        <td className="py-4 px-6">
            <div className="flex items-center gap-2">
                <div className="w-16 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${product.saturation > 70 ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{width: `${product.saturation}%`}}></div>
                </div>
                <span className="text-xs text-slate-500">{product.saturation}%</span>
            </div>
        </td>
        <td className="py-4 px-6">
            <div className="flex items-center gap-2">
                <div className="w-16 bg-slate-800 h-1.5 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-blue-500" style={{width: `${product.competition}%`}}></div>
                </div>
                <span className="text-xs text-slate-500">{product.competition}%</span>
            </div>
        </td>
        <td className="py-4 px-6">
            <span className={`px-2 py-1 rounded-lg text-xs font-bold border ${product.viralScore > 80 ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' : 'bg-slate-800 text-slate-400 border-slate-700'}`}>
                {product.viralScore}
            </span>
        </td>
        <td className="py-4 px-6">
