...
const ProductDrawer: React.FC<ProductDrawerProps> = ({ product, onClose }) => {
  ...
  const handleSave = () => {
      saveIdea({
          ...
          snippet: `Profit: $${product.profit}, Viral Score: ${product.viralScore}, Winning Reason: ${product.winningReason}`,
            ...
            <div className="p-6 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 flex items-center gap-4">
                        <ScoreCircle score={product.viralScore} />
                        <div>
                    ...
                    <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 flex flex-col justify-center">
                         ...
                         <div className="text-2xl font-bold text-white mb-2">{product.saturation}%</div>
                         <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
                             <div className="bg-rose-500 h-full rounded-full" style={{width: `${product.saturation}%`}}></div>
                         </div>
                    </div>
                    <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-2xl font-bold text-white">{product.shopifyStoreCount}</span>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <WhyItWinsCard reason={product.winningReason} benefits={product.benefits} angles={product.angles} />
                    <MarketPotentialCard potentialText={product.marketPotential} saturation={product.saturation} competition={product.competition} />
                </div>
